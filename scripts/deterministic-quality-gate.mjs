...
const main = async () => {
  ...
  if (Number(videoStreams[0]?.width) !== 1080 || Number(videoStreams[0]?.height) !== 1920) {
    issues.push("The final frame size is not 1080 by 1920.");
  }
  if (!Number.isFinite(durationSeconds) || durationSeconds <= 10 || durationSeconds > quality.maximumDurationSeconds) {
    issues.push("The final duration is outside the autonomous Short limit.");
  }
  ...
  const sampledFrames = await listSampledFrames(
    ...
    Number(quality.maximumFrames) || 20,
  );
  ...
  if (!passed) {
    console.error("The deterministic final quality gate rejected the render.");
    process.exitCode = 1;
    return;
  }
  console.log("Deterministic media QC passed; private visual review is required in ChatGPT web.");
};
