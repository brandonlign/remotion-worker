...
assert.equal(validatePrivateRenderContract(completePreview), true);
assert.equal(validatePrivateRenderContract({...completePreview, thumbnailCompositionId: ""}), true);
assert.throws(() => validatePrivateRenderContract({...completePreview, jobFormat: "short"}), /complete long-form preview/);
assert.throws(() => validatePrivateRenderContract({...completePreview, compositionId: "AutoShort"}), /wrong composition/);
assert.throws(() => validatePrivateRenderContract({...completePreview, checkCommand: "true"}), /wrong check command/);
...
const previewScript = fs.readFileSync(new URL("./run-long-preview.sh", import.meta.url), "utf8");
...
assert.match(previewScript, /PREPARE_COMMAND="npm run long:prepare"/);
assert.match(previewScript, /npm run long:contract:test/);
assert.match(previewScript, /status: "long-preview-complete"/);
